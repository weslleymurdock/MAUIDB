...
| COLLATION | yes (can be changed with a rebuild) | string (internally stored as int) | Check [Collation](~/articles/collation/index.md). | `CurrentCulture` and `IgnoreCase` |
...
| --- | --- | --- | --- |
| collation | string | Check [Collation](~/articles/collation/index.md). | null (will use `CurrentCulture` and `IgnoreCase` if null) |
